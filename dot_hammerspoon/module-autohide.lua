...
local log = hs.logger.new("AUTOHIDE")
...
local function handler(name, type, app)
  ...
  if
    type == hs.application.watcher.activated
    or type == hs.application.watcher.deactivated
    or type == hs.application.watcher.hidden
  then
    -- only consider one `affectedSpace`
    local win = app:mainWindow()
    if win then
      for _, spaceId in ipairs(hs.spaces.windowSpaces(win)) do
        if spaceId ~= affectedSpace then
          return
        end
      end
    end
    log.d(name .. " has type " .. type)
...
local function doHide()
  ...
  for i, name in pairs(names) do
    ...
    if i <= minAppsToKeep then
      goto continue
    end
    -- whitelisted apps are always kept
    if contains(whiteList, name) then
      goto continue
    end
    -- inactive for more than `minTimeToKeep` seconds
    -- AND in blacklist or blacklist is empty
    if now - lastActive[name] >= minTimeToKeep and (#blackList == 0 or contains(blackList, name)) then
      local app = hs.application.get(name)
      if app ~= nil then
        local win = app:mainWindow()
        if win ~= nil then
          log.i("Auto hiding " .. app:name())
          app:hide()
        end
      end
